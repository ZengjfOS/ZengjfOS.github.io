...
{      
  ...
  my_syslog(LOG_DEBUG, "enter %s()", __func__);
  ...
  my_syslog(LOG_DEBUG, "option: (%c)", option);
  switch (option)
    { 
    case 'C': /* --conf-file */
      {
...
#  endif
	...
	if (arg && *arg == '/')
	  {
		...
		if (strcmp(arg, "#") == 0)
		  ...
		  option = '?';
		my_syslog(LOG_DEBUG, "domain: %s", domain);
		...
		my_syslog(LOG_DEBUG, "newlist->flags: %s", domain ? "SERV_HAS_DOMAIN" : "SERV_FOR_NODOTS");
		arg = end;
	      }
	    if (!newlist)
	      {
	...
	my_syslog(LOG_DEBUG, "newlist->flags: %08x", newlist->flags);
	...
	if (!arg || !*arg)
	...
	else
	  {
	    ...
	    my_syslog(LOG_DEBUG, "arg: %s, source: %s, source_port: %d, serv_port: %d", arg, source, source_port, serv_port);
	...
	serv = newlist;
	my_syslog(LOG_DEBUG, "serv->domain: %s, serv->next: %p", serv->domain, serv->next);
	while (serv->next)
	  {
  ...
  my_syslog(LOG_DEBUG, "exit %s()", __func__);
...
static void one_file(char *file, int nest, int hard_opt)	
{
  ...
  while (fgets(buff, MAXDNAME, f))
	...
	arg = NULL;
      my_syslog(LOG_DEBUG, "hard_opt: %d, arg: %s", hard_opt, arg);
      ...
      else
	{
      ...
      my_syslog(LOG_DEBUG, "opts[%d].name: %s, option: %d(%c)", i, opts[i].name, option, option);
