			      ...
			      
{
  ...
  for (serv = daemon->servers; serv; serv=serv->next)
	...
	*type = SERV_FOR_NODOTS;
	if (serv->flags & SERV_NO_ADDR)
      ...
      {
        my_syslog(LOG_INFO, _("enter serv->flags & SERV_HAS_DOMAIN"));
	unsigned int domainlen = strlen(serv->domain);
	char *matchstart = qdomain + namelen - domainlen;
	my_syslog(LOG_INFO, _("serv->domain: %s, matchstart: %s"), serv->domain ? serv->domain : "empty domain", matchstart);
	if (namelen >= domainlen &&
	    ...
	    (domainlen == 0 || namelen == domainlen || *(serv->domain) == '.' || *(matchstart-1) == '.' ))
	  {
	    ...
	    *domain = serv->domain;
	    matchlen = domainlen;
	    my_syslog(LOG_INFO, _("matched serv->domain: %s, serv->flags: %08x"), serv->domain, serv->flags);
	    if (serv->flags & SERV_NO_ADDR)
	      flags = F_NXDOMAIN;
	    else if (serv->flags & SERV_LITERAL_ADDRESS)
	      {
        ...
        my_syslog(LOG_INFO, _("exit serv->flags & SERV_HAS_DOMAIN"));
      }
  ...
  my_syslog(LOG_INFO, _("exit %s()"), __func__);
			 ...
			 HEADER *header, size_t plen, time_t now, struct frec *forward)
{
  ...
  if (!daemon->servers)
	...
	flags = search_servers(now, &addrp, gotname, daemon->namebuff, &type, &domain);
      ...
      my_syslog(LOG_INFO, _("flags: %08x"), flags);

      if (!flags && !(forward = get_new_frec(now, NULL)))
	/* table full - server failure. */
	flags = F_NEG;
      
      my_syslog(LOG_INFO, _("forward: %p"), forward);
      if (forward)
	{
	  ...
	  if (type != 0  || (daemon->options & OPT_ORDER))
	    ...
	    my_syslog(LOG_INFO, _("daemon->last_server: %p, start: %p, daemon->servers: %p"), daemon->last_server, start, daemon->servers);
	}
    }
  ...
  if (!flags && forward)
	...
	{ 
	  ...
	  my_syslog(LOG_INFO, _("start->domain: %s"), start->domain ? start->domain : "empyt domain name");
	  if (type == (start->flags & SERV_TYPE) &&
	      ...
	      else
		{
		  ...
		  my_syslog(LOG_INFO, _("daemon->namebuff: %s"), daemon->namebuff);
		  ...
		  my_syslog(LOG_INFO, _("forward->forwardall: %d"), forward->forwardall);
		  if (!forward->forwardall) {
		    my_syslog(LOG_INFO, _("forward->forwardall"));
		    break;
		  }
		  forward->forwardall++;
		}
  ...
  my_syslog(LOG_INFO, _("exit %s() with udpfd send"), __func__);
...
void reply_query(int fd, int family, time_t now)
{
  ...
  my_syslog(LOG_DEBUG, _("enter %s()"), __func__);
