...
static void check_dns_listeners(fd_set *set, time_t now)
{
  ...
  my_syslog(LOG_INFO, _("start check daemon->sfds"));
  for (serverfdp = daemon->sfds; serverfdp; serverfdp = serverfdp->next)
    if (FD_ISSET(serverfdp->fd, set))
      reply_query(serverfdp->fd, serverfdp->source_addr.sa.sa_family, now);
  my_syslog(LOG_INFO, _("end check daemon->sfds"));
  
  my_syslog(LOG_INFO, _("start check daemon->randomsocks"));
  if (daemon->port != 0 && !daemon->osport)
    for (i = 0; i < RANDOM_SOCKS; i++)
      if (daemon->randomsocks[i].refcount != 0 && 
	  FD_ISSET(daemon->randomsocks[i].fd, set))
	reply_query(daemon->randomsocks[i].fd, daemon->randomsocks[i].family, now);
  my_syslog(LOG_INFO, _("end check daemon->randomsocks"));
  
  my_syslog(LOG_INFO, _("start for loop"));
  for (listener = daemon->listeners; listener; listener = listener->next)
    {
