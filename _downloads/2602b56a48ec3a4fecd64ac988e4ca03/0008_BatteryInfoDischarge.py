...
class BatteryInfoDischarge:
    ...
    @input(default/batteryInfo_20221004034502.txt): zcv数据
    """
    ...
    def __init__(self, kwargs):
        ...
        self.lineInfos = LogParser.logFileParser(
                filename,
                r'(.*), soc: (\d*), bat: (\d*\.\d*), vout: (\d*\.\d*), .*, discharge: (\d*\.\d*)',
                callback=self.defaultLineCallback
            )
        for info in self.lineInfos:
            print(info)

        self.labels = ["SOC(%)", "BAT Voltage(mV)", "vout(mV)", "Discharge Current(mA)"]
        self.customData = {"xlabel": "X", "ylabel": "Y"}
        MatplotlibZoom.Show(callback=self.defaultShowCallback, rows = 2, cols = 2)
