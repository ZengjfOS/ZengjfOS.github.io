...
import json
from tools.MatplotlibZoom import figure_pan_and_zoom

import PluginsPy

@PluginsPy.addRun
class MatplotlibZoomExample:
    ...
    def __init__(self, kwargs):
        ...
        if kwargs["show"] == "yes":
            ...
            ax1.plot(xs, cs(xs), label="S")
            # 绘制点以及文本
            for i in range(len(data)):
                # 绘制点
                ax1.plot(X1[i], Y1[i], 'o')
                # 加入字符显示的偏移
                ax1.text(X1[i] - 15, Y1[i] + 0.02, str(X1[i]) + ", " + "{:.2f}".format(Y1[i]), fontsize=9, rotation=90)

            ax1.set_xlabel("X LUX")
            ax1.set_ylabel("Y PWM(0 ~ 255)")
            ax1.set_title("Android (Light Sensor) & (LCD PWM) Mapper")
